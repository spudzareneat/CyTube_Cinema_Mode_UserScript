    ...
    function refreshEmoteData(allowForceRender) {
        ...
        primeEmoteCache(_scEmoteData); // fire-and-forget -- shared cache, see emote-cache module
    ...
    function renderEmoteTile(e, isFav) {
        ...
        // Cached emote images are provided by the shared emote-cache module
        // (src/pc/modules/emote-cache/index.js) -- every emote gets this, not
        // just favorites. Falls back to the live URL until the cache resolves
    ...
    function openEmotesPanel() {
        ...
        if (!_scEmoteData.length) refreshEmoteData(true); // also primes the shared emote cache, see below
        else primeEmoteCache(_scEmoteData); // refreshEmoteData() didn't run above, so prime here instead (fire-and-forget)
