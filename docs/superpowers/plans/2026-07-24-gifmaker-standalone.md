# Standalone GIF Maker Userscript Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a new, standalone Tampermonkey userscript, `cytube.gifmaker.user.js`, that ports the scene-to-GIF feature out of `cytube.pc.user.js` into its own lighter script with a floating record button drawn directly on the video player, hidden during YouTube playback, plus an inline ImgBB key field (no separate settings modal).

**Architecture:** Single self-contained userscript file (matching this repo's existing pattern — see `cytube.pc.user.js`, `cytube.androidTV.user.js`). One IIFE. A `MutationObserver` + polling loop keeps a `◉` button pinned to the corner of `#videowrap`; clicking it lazily injects the GIF-panel CSS and opens a draggable modal (same DOM/CSS approach as the ported code) that captures a hidden crossOrigin `<video>` clone frame-by-frame onto a canvas, burns in optional meme captions, encodes via `gif.js` in a Web Worker, and offers Download / ImgBB Upload.

**Tech Stack:** Vanilla JS (ES2017+, matches existing scripts — no build step, no bundler), Tampermonkey `GM_xmlhttpRequest`/`@require`/`@connect`, `gif.js` 0.2.0 (CDN), Canvas 2D API, `requestVideoFrameCallback`.
...
- **Do not modify `cytube.pc.user.js`** — this is a new, independent file.
- New file path: `C:\Repos\cytube_tv_interface_script\cytube.gifmaker.user.js`.
- `@match` exactly: `https://cytu.be/r/420Grindhouse` and `https://cytu.be/r/testing` (same two channels as `cytube.pc.user.js`).
- `@require` exactly: `https://cdnjs.cloudflare.com/ajax/libs/gif.js/0.2.0/gif.js` (same version pc.user.js uses — a different version's worker protocol may not match).
- ImgBB key localStorage constant must be named `LS_IMGBB = 'sc_imgbb_key'` — this exact string is what `cytube.pc.user.js` already uses, so a key entered in either script is visible to both (same origin).
- The ported GIF panel keeps `cytube.pc.user.js`'s exact `#sc-gif-panel` / `.sc-gif-*` ids and classes (CSS/structure parity with the original, in case anyone cross-references old screenshots/docs). The floating trigger **button** is deliberately given a new, distinct id (`scgm-record-btn`) rather than reusing pc.user.js's `#sc-gif-btn` — pc.user.js's stylesheet has a `#sc-gif-btn { position: fixed !important; ... }` rule that would hijack this script's button positioning if both scripts happen to be installed at once. The panel itself doesn't have this problem: both scripts guard panel creation with `if (document.getElementById('sc-gif-panel')) return;`, so at most one panel instance ever exists, and the two scripts' identical panel CSS is redundant, not conflicting.
- No first-run modal, no onboarding flow, no settings button — the only persisted setting (ImgBB key) lives inline inside the GIF panel itself.

---

### Task 1: Script scaffold — video-corner record button + YouTube-visibility toggle

**Files:**
- Create: `C:\Repos\cytube_tv_interface_script\cytube.gifmaker.user.js`

**Interfaces:**
- Produces: `LS_IMGBB` (string constant), `getKey(id)`/`setKey(id, v)` (localStorage helpers), `getPlayerVideoEl()` (returns the CyTube `<video>` element or null), `isYouTubeMedia()` (returns bool), `openGifPanel(initialSec)` (stub in this task — Task 2 replaces its body; later tasks call it with an optional numeric `initialSec` seconds argument, currently unused).
...
Expected: no output, exit code 0. (This only parses the file — it uses browser globals like `window`/`document` that don't exist in Node, so this step catches typos/syntax errors only, not runtime behavior.)

- [ ] **Step 3: Manual verification in Tampermonkey**

1. Open Tampermonkey's dashboard → **Create a new script** → replace the placeholder with the full contents of `cytube.gifmaker.user.js` → save.
2. Navigate to `https://cytu.be/r/testing`.
3. Open the browser console, confirm you see `[GIFMaker] cytube.gifmaker v1.0.0 loaded`.
4. Confirm a small circular `◉` button appears pinned to the **top-right corner of the video player** (not the browser viewport — resize the window and confirm it stays glued to the video's corner).
5. Hover over the button — confirm it lightens (background/color change).
6. Click it — confirm the console logs `[GIFMaker] openGifPanel stub — panel not implemented yet undefined`.
7. If a YouTube-sourced video is in the playlist/queue, switch to it and confirm the `◉` button disappears within ~1 second; switch back to a locally-hosted file and confirm it reappears.

- [ ] **Step 4: Commit**

```bash
git add cytube.gifmaker.user.js
git commit -m "Scaffold standalone GIF Maker userscript with video-corner record button"
```

---

### Task 2: Frame capture, GIF encoding, meme captions, and the full panel UI (download-only)

**Files:**
- Modify: `cytube.gifmaker.user.js` (replace the `openGifPanel` stub from Task 1)

**Interfaces:**
- Consumes: `getPlayerVideoEl()`, `isYouTubeMedia()`, `getKey`/`setKey`/`LS_IMGBB` (Task 1) — not used yet in this task, but must still be present above.
- Produces: `injectPanelCss()`, `currentRawTitle()`, `gifTitleSlug()`, `_fmtClockTenths(sec)`, `_escHtml(s)`, caption-rendering helpers (`CAPTION_FONT_STACK`, `getCaptionMeasureCtx()`, `wrapCaptionAtSize(ctx, text, fontPx, maxWidth)`, `applyCaptionCtxStyle(ctx, fontPx, color)`, `drawCaptionBlockAdvanced(ctx, w, h, text, color, sizePct, xPct, yPct)`, `drawCaptions(ctx, w, h, captions)`), `computeFrameGeometry(vw, vh, width, aspect)`, `captureGifFrames({src, startT, endT, fps, width, aspect, captions}, onProgress)` (returns a Promise resolving to `{frames, w, h, delay}`), `GIF_WORKER_URL`, `getGifWorkerUrl()`, `getGifCtor()`, `encodeGif({frames, w, h, delay}, onProgress)` (returns a Promise resolving to a `Blob`), `_gifResultUrl` / `_revokeGifResult()`, `getScrubClone(src)` / `destroyScrubClone()` / `grabPreviewFrame(src, time, w)`, and a fully working `openGifPanel(initialSec)` — later tasks (3) will further edit this same function's body.

- [ ] **Step 1: Replace the stub with the full implementation**

In `cytube.gifmaker.user.js`, find this exact block (from Task 1):
...
1. Update the installed script in Tampermonkey with the new file contents, save.
2. Reload `https://cytu.be/r/testing` while a locally-hosted (non-YouTube) file is playing.
3. Click the `◉` button — confirm a draggable "Make GIF" panel opens centered on screen, with START/END preview thumbnails, a duration readout, caption inputs, FPS/Width/Shape selects, and a "● Make GIF" button.
4. Drag the panel by its header — confirm it moves and stays within the viewport.
5. Use "⤓ Now" and the ±.5 buttons on both marks — confirm the duration readout and thumbnails update, and thumbnails show real preview frames from the video (not blank).
6. Type text into both the top and bottom caption fields, switch color to Yellow, change each size percentage — confirm the live preview text overlaid on both thumbnails updates immediately and matches the chosen color/size.
7. Drag the small handle dot on a thumbnail — confirm the corresponding caption's live-preview position moves with it on both START and END thumbnails.
8. Click "● Make GIF" — confirm a spinner with "Capturing frames…" then "Encoding GIF…" progress appears, followed by a GIF preview image and a "⬇ Download" link with a KB size badge.
9. Click Download — confirm a `.gif` file downloads, named after the movie's raw title (e.g. `White-Fire-1984-mkv-<timestamp>.gif`), and that opening it shows the captured clip with captions burned in at the correct position/color/size.
10. Close the panel (✕) and reopen it — confirm no leftover panel/duplicate elements, and a fresh capture still works.

- [ ] **Step 4: Commit**

```bash
git add cytube.gifmaker.user.js
git commit -m "Port GIF capture, encoding, meme captions, and panel UI (download-only)"
```

---

### Task 3: ImgBB key field, validation, and upload

**Files:**
- Modify: `cytube.gifmaker.user.js` (extend the CSS, panel HTML, and `openGifPanel` from Task 2)

**Interfaces:**
- Consumes: `LS_IMGBB`, `getKey`, `setKey` (Task 1); `_escHtml` (Task 2); the exact `openGifPanel` body produced by Task 2 (edits below target specific, verbatim substrings from that task).
- Produces: `validateImgbbKey(apiKey)` (returns a Promise resolving to `'valid' | 'invalid' | 'error'`), `blobToBase64(blob)` (returns a Promise resolving to a base64 string), `uploadToImgbb(blob, apiKey, name)` (returns a Promise resolving to the hosted image URL string).

- [ ] **Step 1: Add the ImgBB functions**

In `cytube.gifmaker.user.js`, find this exact line:

```js
    function openGifPanel(initialSec) {
```

Insert the following directly **above** it (keep `function openGifPanel` as the next line after):
...
- [ ] **Step 2: Add the ImgBB CSS**

Find this exact line inside the `injectPanelCss()` template string (the last rule in it):
...
- [ ] **Step 3: Add the ImgBB row to the panel HTML**

Find this exact line inside `openGifPanel`'s `panel.innerHTML` template:
...
- [ ] **Step 4: Wire the key input and Test button**
...
- [ ] **Step 5: Add the Upload button and wiring to the result view**

Find this exact block (the success path of the `goBtn` click handler):
...
1. Update the installed script, reload `https://cytu.be/r/testing`.
2. Open the GIF panel — confirm an "ImgBB key (for Upload)" field + "Test" button now appears above "● Make GIF", pre-filled if a key was saved previously (e.g. from `cytube.pc.user.js` on the same profile).
3. With the field empty, click Test — confirm "Enter an API key first" in red.
4. Type an invalid string, click Test — confirm "✗ Invalid API key" in red.
5. Type a real ImgBB key (from `https://api.imgbb.com/`), click Test — confirm "✓ Valid API key" in green.
6. Make a GIF, click Upload — confirm it uploads, shows a clickable link, auto-copies to clipboard, and the Copy link button works; button changes to "✓ Uploaded".
7. Clear the key field, blur it (click elsewhere), reopen the panel — confirm the field is now empty (auto-persisted on blur) and Upload on a new GIF shows "Add an ImgBB API key above to enable uploads." without making a network request.

- [ ] **Step 8: Commit**

```bash
git add cytube.gifmaker.user.js
git commit -m "Add inline ImgBB key field, validation, and upload to the GIF panel"
```

---

### Task 4: Full end-to-end verification against the design spec

**Files:** none (verification only — fix inline and commit only if a bug is found)

**Interfaces:**
- Consumes: the finished `cytube.gifmaker.user.js` from Tasks 1–3.
...
- [ ] **Step 2: Run the full testing plan from the design spec**

With `docs/superpowers/specs/2026-07-24-gifmaker-standalone-design.md` open for reference, work through its "Testing plan" section (steps 1–7) exactly as written:

1. Install `cytube.gifmaker.user.js` (and, additionally, `cytube.pc.user.js`) in Tampermonkey, load `https://cytu.be/r/testing`.
2. Confirm the `◉` button stays pinned to the video's top-right corner and survives CyTube re-rendering the video wrapper (e.g. switch to a different queued file).
3. On a non-YouTube file: open the panel, trim a clip, add a top+bottom caption, generate a GIF, confirm download works and the filename reflects the raw movie title.
4. Switch to a YouTube-sourced video; confirm `◉` disappears while it plays, and reappears once a local file resumes.
5. With no ImgBB key saved, click Upload on a generated GIF; confirm the inline hint appears and no request fires (check the Network tab for confirmation).
6. Enter an ImgBB key inline, Test it, save it (blur), regenerate a GIF, Upload, confirm the link/copy button work.
7. With both scripts installed together, confirm an ImgBB key entered in one script's UI is immediately visible/usable in the other's (shared `sc_imgbb_key` localStorage key) — and confirm the two scripts' `◉` buttons don't visually collide or fight over position (this script's button stays pinned to the video corner; pc.user.js's stays fixed to the bottom of the viewport).

- [ ] **Step 3: Fix and re-verify if anything fails**

If any step fails, fix the specific issue in `cytube.gifmaker.user.js`, re-run `node --check`, re-run the failed manual step to confirm the fix, then commit:

```bash
git add cytube.gifmaker.user.js
git commit -m "Fix <short description of the bug found during end-to-end verification>"
```
